...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        ...
        :aria-expanded="open"                        
    >   
        <img class="w-8 h-8 rounded-full border" src="{{ Auth::user()->getMedia('avatar')->first() == null ? asset('img/no-avatar.png') : Auth::user()->getMedia('avatar')->first()->getFullUrl() }}" width="32" height="32" alt="{{ Auth::user()->name }}" />
        ...
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200 dark:border-slate-700">
            <div class="font-medium text-slate-800 dark:text-slate-100">
                @if(Auth::user()->hasRole('Taxista'))
                    <a href="{{ route('taxista.show', Auth::user()->id ) }}">{{ Auth::user()->name }}</a>
                @else
                    <a href="{{ route('empleado.show', Auth::user()->id ) }}">{{ Auth::user()->name }}</a>
                @endif
            </div>
            <div class="text-xs text-slate-500 dark:text-slate-400 italic">{{ Auth::user()->roles()->first()->name }}</div>
        </div>
        <ul>
            <!--<li>
