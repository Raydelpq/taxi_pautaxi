<div x-data="{add:false,
    ...
    <div class="relative bg-indigo-200 dark:bg-indigo-500 p-4 mt-2 sm:p-6 rounded-sm overflow-hidden mb-8">
        ...
        <!-- Content -->
        <div class="relative">
            <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold mb-1">Divisas</h1> 
        </div>
    ...
    <div class="my-3 border shadow rounded-md p-4" x-show="add" x-transition>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 my-2">
            <label for="sigla" class=" col-span-1">Siglas</label>
            <input type="text" class="form-input col-span-1 sm:col-span-2" placeholder="CUP, EUR, USD, MLC" wire:model="divisa.sigla" id="sigla">
            @error('divisa.sigla')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 my-2">
            <label for="valor" class=" col-span-1">Valor</label>
            <input type="text" class="form-input col-span-1 sm:col-span-2" wire:model="divisa.valor" id="valor">
            @error('divisa.valor')
                <span class="text-danger-600 italic col-span-full">{{ $message }}</span>
            @enderror
        </div>
            ...
            @foreach ($divisas as $divisa)
                <tr>
                    <td class="p-2 whitespace-nowrap">
                        <div class="text-left">{{ $divisa->sigla }}</div>
                    </td>
                    <td class="p-2 whitespace-nowrap">
                        <div class="text-left font-medium text-green-500">{{ $divisa->valor }}</div>
                    </td>
                    <td class="p-2 whitespace-nowrap flex flex">
                        <button class="bg-success-500 hover:bg-success-700 text-white mr-1 px-2 py-1 rounded-md" wire:click="edit({{ $divisa->id }})">
                            <i class="fas fa-edit" wire:loading.remove wire:target='edit({{ $divisa->id }})'></i>

                            <svg class="animate-spin h-5 w-6 text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" wire:target="edit({{ $divisa->id }})" wire:loading.class.remove="hidden">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        </button>
                        <button class="bg-red-500 hover:bg-red-700 text-white px-2 py-1 rounded-md" wire:click="delete({{ $divisa->id }})">
